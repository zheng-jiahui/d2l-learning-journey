...
# make_divisible 的核心目的就是为了让通道数（channels / filters）能够被某个数（通常是 8）整除。
def make_divisible(
    v: Union[float, int],
...
class ConvLayer(nn.Module):
    ...
    def __init__(
        ...
        # 分组卷积的组数，默认 1 表示普通卷积。当 groups=in_channels=out_channels 时就是深度可分离卷积。
...
class InvertedResidual(nn.Module):
    ...
    这个类实现了 MobileNetV2 论文中描述的倒残差块。倒残差的意思是：传统残差是"压缩→卷积→扩张"，而这里是"扩张→卷积→压缩"。
    """
    ...
    def __init__(
        ...
        out_channels: int,
        stride: int,
        #扩展因子，通常 >1（如 4 或 6）。中间层的通道数 = in_channels * expand_ratio。
...
class MobileViTBlock(nn.Module):
    ...
    def __init__(
        self,
        in_channels: int,
        transformer_dim: int, #Transformer 处理的维度（通常比 in_channels 大）
        ffn_dim: int,
        n_transformer_blocks: int = 2, #Transformer 编码器的数量，默认 2
        head_dim: int = 32,
        ...
        **kwargs
        ...
        super().__init__()
        #3x3 卷积层（局部特征提取）：输入输出通道数不变，保持分辨率。用于学习局部特征
        ...
        #1x1 卷积层（维度变换）：将通道数从 in_channels 映射到 transformer_dim，为 Transformer 做准备。
        ...
        #1x1 卷积层（维度恢复）：将 Transformer 输出的维度从 transformer_dim 映射回 in_channels。
        ...
        #3x3 卷积层（特征融合）：输入是原始特征和 Transformer 特征的拼接（2倍通道数），输出融合后的特征。
        ...
        #确保 transformer_dim 能被 head_dim 整除。
        assert transformer_dim % head_dim == 0
        num_heads = transformer_dim // head_dim
        #创建多个 Transformer 编码器：列表推导式，循环创建 n_transformer_blocks 个 TransformerEncoder。
        global_rep = [
            TransformerEncoder(
...
#MobileViT 这个主类，它是整个 MobileViT 网络的完整实现。
